...
static void drawFilledPigLine(M5Canvas& canvas, const char* line, int x, int y,
    ...
    static const int8_t openCurve[7]  = {12, 9, 6, 6, 6, 9, 12};
    static const int8_t closeCurve[7] = { 3, 6, 9, 9, 9, 6,  3};
    ...
    if (lineIndex == 1) {
        ...
        int fillLeft = parenLX + openCurve[6];
        int fillRight = parenRX + closeCurve[6];
...
void Avatar::triggerSparkles(uint8_t count) {
    ...
    int cy = 70;             // Middle of pig vertical space (adjusted for Core2 taller canvas)
...
void Avatar::drawFrame(M5Canvas& canvas, const char** frame, uint8_t lines, bool blink, bool faceRight, bool sniff) {
    ...
    if (treePhase == TreePhase::ALIVE || treePhase == TreePhase::GROWING) {
        ...
        while (tbx > 340) tbx -= 420;    // 320px wrap bounds
...
void Avatar::drawGrass(M5Canvas& canvas) {
    ...
    canvas.fillRect(0, snapPx(baseY - 1), 320, PX, color);

    // Pig body footprint for grass bending
    ...
    // Draw fruit splash particles (fat-pixel squares, grid-snapped)
    for (uint8_t i = 0; i < FRUIT_SPLASH_COUNT; i++) {
        if (!fruitSplashes[i].active) continue;
...
void Avatar::waveRipple(WaveMode mode) {
    ...
    if (mode == WaveMode::INCOMING && waveMode == WaveMode::OUTGOING) {
        if (millis() - waveBurstStart < 1500) return;
    }
    waveMode = mode;
